...
def generate_inflation_plot_3_countries(dataset,country1,country2,country3): 
    ...
    plt.title('Inflations of Afghanistan,Myanmar,Nigeria')
    plt.xticks(rotation=45)
    
    #Save as png, bbox_inches makes sure labels are not cut out.
    plt.savefig('line_graph_comparing_inflations.png',
                bbox_inches='tight',
                dpi=200)
    plt.show()
...
def generate_bar_chart_inflation_sicnce_start(dataset):
    ...
    plt.savefig("total_increase_inflation.png",
                bbox_inches='tight', 
                dpi=200)
    ...
    #Save figure as png.
    plt.savefig("increase_in_inflation_line_graph.png",
                bbox_inches='tight', 
                dpi=200)
    plt.show()
    
    #Removing the top 4 countries from the dataset to visualise other countries.
    temp2 = temp_df.query(
    ...
    #Save figure as png.
    plt.savefig("Inflation since start (excluding last 4 countries)", 
                bbox_inches='tight', 
                dpi=200)
    plt.show()
...
def generate_pie_chart_transaction_types(dataset):
    ...
    plt.savefig("pie_chart_transaction_type.png",
                bbox_inches='tight', 
                dpi=200)
...
generate_bar_chart_inflation_sicnce_start('world_food_inflation_details.csv')
